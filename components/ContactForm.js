...
export default function ContactForm({ dict }) {
    ...
    const handleSubmit = (e) => {
        ...
        // Temporarily set to unavailable as requested
        setTimeout(() => {
    ...
    if (formStatus === "unavailable") {
        return (
            <div className="bg-primary p-12 rounded-3xl text-center border border-secondary/30 min-h-[500px] flex flex-col justify-center">
                <div className="w-20 h-20 bg-amber-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
                    <svg className="w-10 h-10 text-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                </div>
                <h3 className="text-2xl font-bold text-white mb-4">{dict.form.unavailableTitle}</h3>
                <p className="text-gray-300 mb-8 max-w-sm mx-auto">{dict.form.unavailableDesc}</p>
                <button
                            ...
                            <div className="flex items-start">
                                ...
                                <div>
                                    ...
                                    <div className="mt-2">
                                        ...
                                        <p className="text-gray-600 text-sm">{dict.president.phone}</p>
                                        <p className="text-gray-600 text-sm">{dict.president.email}</p>
                                    </div>
                                    <div className="mt-4">
                                        <p className="text-primary font-bold">{dict.pm.title}: {dict.pm.name}</p>
                                        <p className="text-gray-600 text-sm">{dict.pm.phone}</p>
                                        <p className="text-gray-600 text-sm">{dict.pm.email}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
